// PrintFarm — shared printer probing logic.
//
...
/**
 * Prefer the printer's percentage, but recover from Creality telemetry that
 * leaves it at 0 while valid layer counters continue to advance.
 */
...
export function probe(printer, timeoutMs = 4000) {
  return new Promise((resolve) => {
    ...
    const done = (status, job) => {
      ...
      try { ws?.close(); } catch {}
      resolve({ ...printer, status, job, rawMessages });
    };
    ...
    // Every frame exactly as the printer sent it, for the dashboard's raw view.
    // The accumulator below only keeps fields this adapter already knows about,
    // so it cannot show an unlisted printer's unrecognized field names.
    const rawMessages = [];
    const RAW_LIMIT = 40;
    ...
    ws.addEventListener("message", ({ data }) => {
      try {
        ...
        if (rawMessages.length < RAW_LIMIT) rawMessages.push(msg);
...
export function judge(printer) {
  ...
  const progress = derivePrintProgress(state.printProgress, state.layer, state.totalLayer);
