...
// Manual overrides: Map of printer IP -> "needs_clearing" or "free"
export const manualOverrides = new Map();

/**
 * Treat idle printers as physically unverified after startup or discovery.
 * Firmware "free" only means no job is running; it cannot tell whether a
 * finished part is still sitting on the bed.
 */
export function requireBedClearance(printers) {
  for (const printer of printers) {
    if (!printer?.ip) continue;

    if (printer.farmState === 'busy' || printer.farmState === 'paused') {
      manualOverrides.delete(printer.ip);
      continue;
    }

    manualOverrides.set(printer.ip, 'needs_clearing');

    const current = [...farmState.values()].find(state => state.ip === printer.ip);
    if (current && (current.farmState === 'free' || current.farmState === 'needs_clearing')) {
      current.farmState = 'needs_clearing';
    }
  }
}
...
export function startFarmPolling(initialPrinters, intervalMs = 5000) {
  ...
  async function poll() {
    ...
    for (const p of activePrinters) {
      ...
      // An online socket without a state field is not evidence of a transition.
      // Preserve the last trusted snapshot so an incomplete probe cannot make a
      // printing machine eligible for another dispatch.
      if (raw.status === 'online' && !final.telemetryComplete && previous) {
        // The safety state stays frozen, but the raw frames must not. A printer
        // whose telemetry this adapter cannot read never reports complete
        // telemetry, so freezing those too would leave the raw view stuck on
        // whatever the printer happened to be doing at startup — exactly when
        // someone needs to watch it print in order to add support for it.
        previous.rawMessages = final.rawMessages;
        continue;
      }

      // If a printer was busy and is now free, lock it into needs_clearing.
      if (previous && previous.farmState === 'busy' && final.farmState === 'free' && final.telemetryComplete) {
      ...
      if (
        override === 'needs_clearing'
        && (final.farmState === 'free' || final.farmState === 'needs_clearing')
      ) {
        final.farmState = 'needs_clearing';
      } else if (
        override === 'needs_clearing'
        && (final.farmState === 'busy' || final.farmState === 'paused')
      ) {
        manualOverrides.delete(final.ip);
